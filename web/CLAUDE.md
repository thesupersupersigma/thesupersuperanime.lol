...
- **`preferNativeHLS` is set on `<MediaPlayer>` for Safari ONLY** (`preferNativeHLS={isSafari}`, where `isSafari` is a stable SSR-safe `useState` lazy-init flag). It was previously set globally and removed (commit `026bca3`) because it caused a freeze-on-resume bug — but the resume path now has a dedicated native-HLS branch in `onCanPlay` (`isNative` → seek), so Safari-only native HLS is safe. **Why Safari-only:** Safari can't composite hls.js/MSE-driven video in fullscreen (black screen, audio continues — a WebKit limitation); playing HLS natively routes fullscreen through `webkitSetPresentationMode` and renders correctly. `preferNativeHLS` only reorders Vidstack's provider loaders — on Chrome/Firefox the native `<video>` loader declines HLS (no native support) so they keep using hls.js **unchanged**. On Safari the HLS source `type` is also set to the canonical `application/vnd.apple.mpegurl` (vs `application/x-mpegurl` elsewhere) to guarantee the native loader claims it. **Do NOT set `preferNativeHLS` globally / unconditionally** — that reintroduces the original bug on non-Safari browsers.
