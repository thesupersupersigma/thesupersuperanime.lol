...
Tests use Node's built-in runner — no framework, no extra dependencies:

```bash
npm test        # unit tests: src/**/*.test.mts
npm run test:db # integration tests against Neon: tests/**/*.test.mts (skips without DATABASE_URL)
npm run test:all
npm run typecheck
```

Test files are `.mts` (always ESM) and import sources with an explicit `.ts` extension — `allowImportingTsExtensions` is on, and `noEmit` makes that legal. Node's type stripper rejects TS-only *runtime* syntax, so no parameter properties (`constructor(private x)`), no enums, no namespaces in anything a test imports.

DB tests run inside an interactive transaction that is **always rolled back**, so they leave no rows behind. Several carry an explicit regression guard that performs the *pre-fix* behaviour and asserts it still fails — keep that pattern; a test that only exercises the happy path can pass vacuously.

TypeScript build errors are **no longer ignored** (`ignoreBuildErrors: false`) — `next build` typechecks, matching what `ci.yml` already ran.
...
- **`src/lib/core-dist/`** — pre-built `@tsss/core` module (referenced via `src/lib/core.ts`). Today it's only the legacy provider registry (`gogoanime`, `aniwave` — both pinned to domains that died in 2024) used by the admin health-check dashboard. `getRacedSources` is still exported but is **dead code**. Its `checkRateLimit` is also no longer used: the eviction branch was unreachable so the Map grew forever, and both call sites moved to `src/lib/rate-limit.ts`. Don't edit `core-dist/` — it's a committed build artifact; fix things in `src/lib/` and move the call sites.
- **Video sources** come from the external **RECONSUMET-TS** scraper at `SCRAPER_API_URL`, not from the bundled providers.
...
5. **Discord/email verification gate** — verifies the `user-session` cookie's HMAC (see below); gates most remaining routes, **including `/admin/*` and `/api/admin/*`** (they are not in the exempt list). Disable entirely with `DISCORD_GATE=off`. `MASTER_GATE=off` lets anonymous (no `user-session` cookie) users browse without a session. Note the proxy does **not** read `discord-linked` / `email-verified` cookies — that cookie mirroring was removed when the forgeable-gate bug was fixed; Discord/email verification is enforced server-side (DB-backed) by the `(site)` layout + `DiscordGateCheck`. Exempt paths: `/account*`, `/api/auth/*`, `/api/import/*`, `/api/watchlist/*`, `/api/progress/*`, `/leaderboard`, `/status`, `/user/*`.
...
The `user-session` cookie is **`<opaque token>.<hex HMAC-SHA256>`**, signed with `SESSION_COOKIE_SECRET` (falling back to `TOKEN_SECRET`). The proxy runs on the Edge and can't reach Postgres, so it verifies that signature — it used to accept the mere *presence* of the cookie, which meant `document.cookie = "user-session=x"` walked past the gate. The signature only proves the server minted the token; `getCurrentUser()`'s DB lookup still decides whether the session is real and unexpired. Signing/verifying lives in `src/lib/session-cookie.ts` and uses Web Crypto so the same code runs on Edge and in Node. It **fails closed**: no secret means no session verifies. Unsigned (pre-signing) cookies are rejected, so those users sign in again.

Auth helpers live in `src/lib/auth.ts` (scrypt password hashing, no bcrypt). Secret comparisons are constant-time: `safeCompare()` in `auth.ts` (Node routes reuse it, e.g. `/api/anilist/sync/auto`), and a local `timingSafeEqualString()` in `src/proxy.ts` (the proxy runs on the Edge and can't use Node `crypto`). Auth server actions (`src/app/account/actions.ts`) have a best-effort in-memory rate limiter: sign-in 5/15min per email (never reset on success), sign-up 5/hr per IP (`x-forwarded-for`), password-reset requests 3/hr per email (over-limit silently returns `{ success: true }` to avoid confirming email existence). `isAdmin(discordId)` checks the Discord ID against the `ADMIN_1`, `ADMIN_2`, ... env var allowlist (sequential, stops at the first missing index) — this is the **sole** admin authorization mechanism.

## Video Source Pipeline (RECONSUMET-TS)

`POST /api/source` (`src/app/api/source/route.ts`) is the core endpoint. It talks to the external **RECONSUMET-TS** scraper at **`SCRAPER_API_URL`** (`api.thesupersuperanime.lol`). There is no "Anivexa" API and no `ANIVEXA_API_URL` in this path — that variable is legacy and unread.

1. **Auth** — `requireAuth()` in-route when `DISCORD_GATE`/`MASTER_GATE` are on. The proxy gate can only verify the session cookie's signature; this is the DB-backed check.
2. **Rate limit** — `checkRateLimit()` from `@/lib/rate-limit` (10 req/60s), keyed on `source:user:{id}` when signed in, else `source:ip:{ip}`. Deliberately *not* the `session-id` cookie: that's client-supplied, so rotating it gave a fresh bucket every request, and its `"anonymous"` fallback put every first-time visitor in one shared bucket.
3. **Provider discovery** — `GET {SCRAPER_API_URL}/info/{animeId}` (mappings, highest score first) and `GET {SCRAPER_API_URL}/episodes/{animeId}` (the aggregator's own pick) fire concurrently.
4. **Episode lists** — every provider in the queue that doesn't already have episodes gets `GET /episodes/{animeId}?provider={name}`. The pending set is computed from what's actually held, never from queue position.
5. **Provider verification** — a secondary `/episodes` response is only filed under the provider **the response says answered** (`data.provider`), never the one requested. An episode id is only meaningful to its own provider (AniNeko emits `<slug>/ep-N`, ReAnime expects `<anilistId>/<ep>`), so a mis-filed list produced 400s on the following `/watch` call and made a provider appear to randomly vanish. A response naming no provider is trusted (older API builds don't report one).
6. **Streams** — `GET /watch?provider=&episodeId=` per pair. Each audio type (`sub`/`dub`) returns an **array** of per-server results; every server is kept.
7. **Grouping** — one entry per `provider:type:serverName`, so multi-server providers stay distinguishable in the dropdown (`Anikototv — HD-1`). There is **no** priority filter and nothing is dropped.
8. Subtitles are **deduped by language** (`dedupedSubtitles`) since a provider can return multiple tracks for the same language.
9. Each stream URL + cookies + referer is JSON-encoded, encrypted with AES-256-CBC (`ENCRYPTION_SECRET`), and the resulting token is HMAC-SHA256 signed (`TOKEN_SECRET`). `SourceToken` rows are stored in the DB with a **3-hour expiry** (covers a full viewing session including pauses).
10. Response shape: `{ servers: [{ name, type, sources, subtitles }], fallbackReason }`. There is **no** `mirrorUsed` field — client props of that name are vestigial. The 404 body also carries `fallbackReason` (`primary` / `not_found` / `timeout` / `error`) so "upstream timed out" is distinguishable from "this episode has no sources".

The pure pairing rules live in `src/app/api/source/provider-pairing.ts` and are unit-tested (`provider-pairing.test.mts`) — change them there, not inline in the route.
...
- Segment/key token ids are **deterministic**: `HMAC(TOKEN_SECRET, "<parent token>|<url>")` truncated to 48 hex (`src/lib/segment-token.ts`), written with `createMany({ skipDuplicates: true })`. Playlist tokens are `isM3U8` so the single-use check never applies and one token is replayable for its full 3h life — with the old `randomBytes(24)` ids every replay minted a fresh batch of rows (250–1000 per episode view). Keep this deterministic; making it random again restores the write amplifier. URI-bearing lines per playlist are capped at 10,000 (502 past that).
...
`GET /api/subtitle-proxy?url=...` (`src/app/api/subtitle-proxy/route.ts`) — public (exempted in `src/proxy.ts`), fetches `.vtt` subtitle files from an **allowlist of hosts** (`mewstream.buzz`, `megaplay.buzz`, `vidwish.live`, `anineko.to`, `anikototv.to`, `cdn.mewstream.buzz`, `s.megaplay.buzz`, `lostproject.club`, `watching.onl`, plus subdomains) with a spoofed `User-Agent`/`Referer`/`Origin` (`megaplay.buzz`). Returns `text/vtt` with CORS allowing `NEXT_PUBLIC_SITE_URL`. Bodies are capped at **2 MB** (`readCappedText` in `src/lib/read-capped.ts`) — Content-Length is checked first, then bytes are counted as they arrive and the upstream body is cancelled past the cap. The route is public, unauthenticated and unrated, and the allowlist constrains only the *hostname*, so an uncapped `res.text()` was an OOM vector. The player references subtitle tracks via this proxy (`<Track src="/api/subtitle-proxy?url=...">`), not the raw provider URL — adding a new subtitle CDN requires adding its host here. This feature is still actively being worked on.
...
**Only the cron calls `runStatusChecks()` in `src/lib/status.ts`** (public reads use the read-only `getStatusSnapshot()`), which: (1) live-pings four infrastructure services in parallel with `AbortSignal.timeout(5000)` — `site` (`GET {SITE}/api/auth/me`, any response = up, even 401), `database` (`db.user.count()`, up only if under 3s), `scraper` (`GET {SCRAPER_API_URL}/health`, any response = up — **no fallback host**; an unset variable reads `unknown` with a logged config error, writes no `StatusCheck` row and opens no incident, rather than reporting a permanent outage), `anilist` (POST GraphQL, up only on 2xx); (2) reflects the latest `ProviderStatus` rows (read from DB, **not** re-scraped — `healthy→operational`, `degraded→degraded`, `broken→outage`) as `providers`-group services keyed by raw `providerId`; (3) `createMany`s one `StatusCheck` per service (`success` = operational OR degraded); (4) opens an incident when a service reads `outage` with no open incident, auto-resolves (`resolvedAt`, `autoResolved=true`) when it reads `operational` with one open — degraded/unknown leave incidents untouched; (5) returns `{ services[], overallStatus, lastUpdated, incidents: { opened, resolved } }` where each service carries `status`, `latencyMs`, `uptimePercent` (last 90 checks, 1 decimal), a 90-slot `history` array (oldest→newest, `null` = no data), and an optional open `incident`. `overallStatus` = worst of the **infrastructure** services only (outage > degraded > operational). The `providers` group is deliberately excluded: it reflects the legacy `core-dist` registry (`gogoanime`/`aniwave`, domains dead since 2024, sitting at 66 consecutive fails), and letting it vote made the banner read "Major Outage" permanently no matter how healthy the site was. The pure rules are in `src/lib/status-rules.ts` (unit-tested); `scripts/purge-stale-status.mjs` cleans up history left by the old `anivexa` service id (dry-run by default).
...
- `GET /api/cron/status-check` (`src/app/api/cron/status-check/route.ts`) — `CRON_SECRET` Bearer (re-checked in-route), calls `runStatusChecks()`, returns `{ checked, incidents }`. **This is the only caller of `runStatusChecks()`** — `/api/status` and `/status` both use the read-only `getStatusSnapshot()`, so public reads never ping or write. Runs every 5 min via `.github/workflows/health-check.yml` (renamed "Health & Status Checks"), which now has two cron schedules — the existing 12h provider `health-check` POST and the new 5-min status-check GET, gated per-step by `github.event.schedule` (manual `workflow_dispatch` runs both).

## Agent & Machine Discovery

The site advertises itself to AI agents / crawlers via a set of discovery resources. Most are static files under `public/`; the dynamic pieces are in `src/proxy.ts` and `src/app/api/md/route.ts`.

- **`Link` header (RFC 8288)** — set in `src/proxy.ts` (`DISCOVERY_LINK` + `withDiscovery()`), **not** `next.config.ts`, so it survives the auth-gate redirect an unauthenticated agent gets on `/` (attached to the `/login` + `/account` redirects and every non-`/api/` page response; JSON API responses are left clean). Points to the sitemap, llms.txt (describedby + service-doc), api-catalog, openapi.json (service-desc), `/api/status` (status), MCP server card, and agent-skills.
- **`public/robots.txt`** — now a **static file** (the old `src/app/robots.ts` metadata route was deleted so arbitrary directives are possible). Carries a `Content-Signal: search=yes, ai-input=yes, ai-train=yes` line (contentsignals.org) alongside the usual allow/disallow + `Sitemap:`.
- **`public/llms.txt`** — plain-text site overview for LLMs.
- **`public/.well-known/api-catalog`** — RFC 9727 API catalog, served as `application/linkset+json` (content-type forced via `next.config.ts` `headers()` since the file is extensionless). Anchors the public API and links service-doc (llms.txt), service-desc (openapi.json), status (`/api/status`), describedby (MCP card).
- **`public/.well-known/openapi.json`** — OpenAPI 3.1 describing the public **read-only** endpoints only: `/api/status`, `/api/announcement`, `/api/changelog`, `/api/chat/channels`, `/api/chat/online`, `/api/chat/{roomId}`, `/api/watch-party/{roomCode}`.
- **`public/.well-known/mcp/server-card.json`** + **`public/.well-known/agent-skills/index.json`** — MCP server card and agent-skills index (pre-existing).
- **Markdown for Agents** — `src/proxy.ts` does Accept-based content negotiation: a `GET` whose `Accept` explicitly contains `text/markdown` (browsers never send that) for a non-`/api/` path is internally **rewritten** to `src/app/api/md/route.ts`, passing the original path via an **`x-md-path` request header** (a query param does **not** survive a middleware rewrite — the destination's `req.nextUrl` still shows the original URL). That route returns `text/markdown`: curated markdown for key pages (`/`, `/search`, `/genres`, `/leaderboard`, `/status`, `/updates`, `/privacy`, `/terms`) and a generic fallback otherwise. The homepage markdown mirrors `llms.txt` — keep the two in sync. `/api/md` is in the proxy's always-public list; markdown-negotiable page responses set `Vary: Accept`.

**Deliberately not implemented:** OAuth/OIDC discovery (`openid-configuration`, `oauth-authorization-server`, `oauth-protected-resource`) and `auth.md` — the site *consumes* Discord/AniList OAuth but is not itself an authorization or resource server, so publishing that metadata would advertise endpoints that don't exist. DNS-AID records are a DNS/DNSSEC concern (registrar/DNS host), not code.
...
- **Watch progress**: Dual-tracked — per authenticated user (`userId`) or per anonymous session (`sessionId`); stored as milliseconds in `WatchHistory`. Rows owned by a logged-in user store `sessionId` as **`u:<userId>`** (`src/lib/owner-session.ts`), never the browser's `session-id`: `WatchHistory`/`Watchlist` each carry a second unique index on `(sessionId, key)`, so two accounts sharing one browser collided and the second account's create raised `P2002` on every save, forever. `destroyUserSession()` also rotates `session-id` so a browser doesn't carry one account's id into the next. `GET /api/progress` dedupes to one row per anime (latest episode) when returning history; with `?episodeId=...` it instead does an exact single-row lookup (same `{ history: [...] }` shape, 0–1 items) — used by the watch page for the resume position.
...
- `SCRAPER_API_URL` — base URL for the RECONSUMET-TS scraper (see Video Source Pipeline). Also what the status page pings. `ANIVEXA_API_URL` is legacy and read by nothing.
- `SESSION_COOKIE_SECRET` — HMAC key for the `user-session` cookie signature; optional, falls back to `TOKEN_SECRET`. Changing it signs everyone out.
- `DISCORD_WEBHOOK_URL` — provider health failure/recovery alerts (distinct from `DISCORD_ALERT_WEBHOOK_URL`, which is the OAuth-callback security embed)
