...
exports.StreamtapeExtractor = void 0;
/**
...
class StreamtapeExtractor {
    ...
    async extract(embedUrl) {
        ...
        const robotLinkMatch = html.match(/id\s*=\s*["']robotlink["'][^>]*>([^<]+)<\//);
        if (!robotLinkMatch) {
        ...
        const scriptMatch = html.match(/getElementById\s*\(\s*['"]robotlink['"]\s*\)[\s\S]*?\+\s*\('([^']+)'\)\.substring\((\d+)\)/);
        if (!scriptMatch) {
